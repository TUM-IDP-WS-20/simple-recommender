...
Bootstrap(lrs_app)
lrs_app.config.from_object(os.environ['APP_SETTINGS'])
lrs_app.config['SQLALCHEMY_DATABASE_URI'] = os.environ['DATABASE_URL']

lrs_app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(lrs_app)
...
@lrs_app.route('/', methods=['GET', 'POST'])
def index():
    is_rated = False
...
if __name__ == '__main__':
    ...
    lrs_app.run(threaded=True, port=5000)
